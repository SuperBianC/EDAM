...
class ResUNet(nn.Module):
    ...
    def forward(self, inputs):
        #print('*'*25,'input_size:',inputs.size(),'*'*25)
        #print('*'*25,'encoder_stage1_size:',self.encoder_stage1(inputs).size(),'*'*25)
        ...
        long_range2 = F.dropout(long_range2, drop_rate, self.training)

        short_range2 = self.down_conv2(long_range2)

        long_range3 = self.encoder_stage3(short_range2) + short_range2
        long_range3 = F.dropout(long_range3, drop_rate, self.training)

        short_range3 = self.down_conv3(long_range3)

        long_range4 = self.encoder_stage4(short_range3) + short_range3
        long_range4 = F.dropout(long_range4, drop_rate, self.training)

        short_range4 = self.down_conv4(long_range4)

        outputs = self.decoder_stage1(long_range4) + short_range4
        outputs = F.dropout(outputs, drop_rate, self.training)
